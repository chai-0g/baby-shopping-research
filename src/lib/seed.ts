...
import { getDb, queryOne, execute } from './db';
...
export async function seedDatabase() {
  ...
  const catCount = queryOne(db, 'SELECT COUNT(*) as count FROM categories') as { count: number } | undefined;
  if (catCount && catCount.count > 0) return { seeded: false, message: 'Database already seeded' };

  const itemIdMap: Record<string, string> = {};

  seedData.forEach((cat, catIdx) => {
    execute(db, 'INSERT INTO categories (id, name, icon, sortOrder) VALUES (?, ?, ?, ?)', [cat.id, cat.name, cat.icon, catIdx]);
    cat.items.forEach((item, itemIdx) => {
      const itemId = uuid();
      itemIdMap[item.name] = itemId;
      execute(db, 'INSERT INTO items (id, categoryId, name, priority, status, notes, paperclipIssueId, sortOrder) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [itemId, cat.id, item.name, item.priority, item.status, item.notes, item.paperclipIssueId || null, itemIdx]);
    });
  });

  // Dependency chain: Mattress depends on Cot, Sheets depends on Mattress
  if (itemIdMap['Mattress'] && itemIdMap['Cot/Crib/Co-sleeper']) {
    execute(db, 'INSERT INTO dependencies (id, itemId, dependsOnItemId, dependencyType, description, status) VALUES (?, ?, ?, ?, ?, ?)',
  ...
  return { seeded: true, message: `Seeded ${seedData.length} categories with ${seedData.reduce((s, c) => s + c.items.length, 0)} items` };
}
